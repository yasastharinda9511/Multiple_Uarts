...
void MULTIPLE_UART::Uart_init(UART_HandleTypeDef* uart1){
	  uart=uart1;
	  rx_buffer = { { 0 }, 0, 0};
	  tx_buffer = { { 0 }, 0, 0};
...
void MULTIPLE_UART::store_char(unsigned char c, ring_buffer *buffer){
...
void MULTIPLE_UART::Uart_sendstring (const char *s)
{
	while(*s) Uart_write(*s++);
}

void MULTIPLE_UART::Uart_write(int c)
{
	if (c>=0)
	{
...
int MULTIPLE_UART::Uart_read(void)
{
...
int MULTIPLE_UART::IsDataAvailable(void)
{
  return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer->head - _rx_buffer->tail) % UART_BUFFER_SIZE;
}

uint16_t MULTIPLE_UART:: get_pos (char *string)
{
...
void MULTIPLE_UART::Uart_printbase (long n, uint8_t base)
{
...
void MULTIPLE_UART::Get_string (char *buffer)
{
	int index=0;
...
int MULTIPLE_UART::wait_until (char *string, char*buffertostore)
{
